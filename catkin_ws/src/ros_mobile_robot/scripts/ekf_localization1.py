...
from fusionEKF1 import FusionEKF1
...
if __name__ =='__main__':
    dim_x = 3
    noise_x = 0.025 #ok:0.025  |no: 0.1 cang nho cang gan uwb, lon -> gan odom
    noise_y = 0.025 #0.025
    noise_yaw = np.deg2rad(25) #20 ############################# 0.07, 20, .05, .001: ekf theo uwb, gocs leechj nhieu, doan dau // odom, ve sau cang sai
    noise_v = 0.03 ############
    noise_w = 0.02 ###########
    P = np.diag([.01, .01, .001])**2 ##################.01 .01 .001
    Q = np.diag([noise_v, noise_w])**2 ######################
        
    #IMU
    ekf_imu = FusionEKF1(dim_x=dim_x, dim_z=1)
    ekf_imu.P = P
    ekf_imu.Q = Q
    ekf_imu.R = np.diag([noise_yaw])**2

    #UWB
    ekf_uwb = FusionEKF1(dim_x=dim_x, dim_z=2)
    ekf_uwb.P = P
    ekf_uwb.Q = Q
    ekf_uwb.R = np.diag([noise_x, noise_y])**2
    
    #UWB+IMU
    ekf_all = FusionEKF1(dim_x=dim_x, dim_z=3)
    ekf_all.P = P
    ekf_all.Q = Q
