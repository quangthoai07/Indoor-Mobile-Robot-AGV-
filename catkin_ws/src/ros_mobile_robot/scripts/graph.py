...
clicked_points1 = []
clicked_points2 = []
last_imu_callback_time = 0
last_vel_callback_time = 0
last_odom_callback_time = 0
last_ekf_callback_time = 0
last_filtered_yaw = None
time_data = []  
a = 0    
def subscriber_vel_callback(vel_data):
    global vel
    global last_vel_callback_time
...
def subscriber_ekf_callback(ekf_data):
    global x_EKF, time_data, last_filtered_yaw
    ...
    yaw = PyKDL.Rotation.Quaternion(rot.x, rot.y, rot.z, rot.w).GetRPY()[2]
#    filtered_yaw = lowpass_filter_single(yaw, last_filtered_yaw)
#    last_filtered_yaw = filtered_yaw
    
#    pose = np.array([[x], [y], [filtered_yaw]])
    pose = np.array([[x], [y], [yaw]])
    x_EKF = np.hstack((x_EKF, pose))
    x_EKF = x_EKF.tolist()
    x_EKF[2] = [normalize_angle(angle) for angle in x_EKF[2]]
#    x_EKF[2] = unwrap_angles(x_EKF[2])
...
def lowpass_filter_single(new_angle, last_angle, alpha=0.1):
    if last_angle is None:
        return new_angle
    return alpha * new_angle + (1 - alpha) * last_angle
...
def show_plot(i, ax1, ax2):
    ...
    global vel, previous_time, fig, time_data
...
#    ax2.plot(np.rad2deg(x_DR[2]), '-k', linewidth = linewidth, label='th_DR')#######################################
    ...
    for point in clicked_points2:
        ...
        ax2.text(point[0], point[1], f'({point[0]:.2f}, {point[1]:.2f})', fontsize=10, color='black', ha='left', va='bottom')
...
def main():
    ...
    ani = FuncAnimation(fig, show_plot, fargs=(ax1, ax2), interval=100)##############################
